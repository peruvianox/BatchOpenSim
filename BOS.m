...
%% Settings
cd('C:\Users\richa\Documents\Packages\BOS');
...
Settings.Scale = 'Yes';          % model scaling
...
%     subjectPath = 'D:\UNC_ABL\FpMetabolics_2020\SubjData';
    subjectPath = 'D:\UNC_ABL\AgeFatigue';
...
for subj = 1:length(Subjects) % Subject Loop
    ...
    WalkingTrials = logical(strcmp({Subjects(subj).Trials.type}, 'walking'));
    
    for trial = find(WalkingTrials) % Trial Loop
