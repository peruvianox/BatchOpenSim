...
%     RightVertGRF = contains(GRF.ColHeaders, 'ground_force_vz');
    RightVertGRF = find(strcmp(GRF.ColHeaders, 'ground_force_vy'));
...
VertMaxThresh = 400; % vGRF thresh to delete invalid steps
VertMinThresh = 100;
...
for i = 1:TSData.L_NumOffs - 1
    ...
    Step = resample(step, 100, length(step));
    if length(Step) ~= 100
        ToDel(i) = 1;
        continue
    end
    if max(Step) < VertMaxThresh
        ToDel(i) = 1;
    end
    if min(Step) > VertMinThresh
        ToDel(i) = 1;
    end
    
    % exclude steps based on timing of peaks
    [~, Inds] = findpeaks(Step, 'MinPeakHeight', PkHt, 'MinPeakProminence', PkProm);
    if length(Inds) ~= 2
        ToDel(i) = 1;
        continue
    end
