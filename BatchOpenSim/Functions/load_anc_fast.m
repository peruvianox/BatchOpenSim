function [anc, time, samp_rate, channel_names, range, voltdata] = load_anc_fast(infile,inpath)
...
%% Load channel data
fscanf(fid,'%s',1);
samp_rate = zeros(1, num_channels);
range = zeros(1, num_channels);
for i = 1:num_channels
...
while feof(fid)==0 && isempty(linetemp) == 0
    time(i,1)=linetemp(1);
